...
to setup
  ...
  set species.data (csv:from-file "parameters.csv" file.delimiter)                                  ; import csv file into a list of lists
  ...
  foreach n-values nr.species [? + 1] [
    ...
    create-fishes (sp.density * actual.area) [
      ...
      if reduce + behavior.freqs != 1 [
       user-message (word "ERROR! Behavior frequencies for " species " don't add up to 1.")
       stop
      ]
  ...
  output-print (word "Imported " count fishes " fishes belonging to " nr.species " species, with a total density of " (count fishes / actual.area) " fish per square meter")
  reset-ticks
end
@#$#@#$#@
GRAPHICS-WINDOW
